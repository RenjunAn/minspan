...
def run_task_with_injection_tasks(
    ...
    for injection_task_id in injection_tasks_to_run:
        ...
        if not force_rerun and logdir is not None and agent_pipeline.name is not None:
            try:
                task_result = load_task_results(
                    agent_pipeline.name.replace("/", "_"),
                    suite.name,
                    user_task.ID,
        ...
        if task_result is not None and not force_rerun:
            ...
            with TraceLogger(
                ...
                try:
                    ...
                    logger.log_error(
                        f"Skipping task {user_task.ID} with '{injection_task.ID}' because of JSON decode error: {e}"
                    )
                    utility = False
                    security = True
...
_TASK_RESULTS_MODEL_REBUILT = False


def _ensure_task_results_model_rebuilt() -> None:
    global _TASK_RESULTS_MODEL_REBUILT
    if _TASK_RESULTS_MODEL_REBUILT:
        return
    TaskResults.model_rebuild(_types_namespace={"FunctionCall": FunctionCall})
    _TASK_RESULTS_MODEL_REBUILT = True
...
def load_task_results(
    ...
    _ensure_task_results_model_rebuilt()
