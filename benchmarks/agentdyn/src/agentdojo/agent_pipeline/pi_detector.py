...
class TransformersBasedPIDetector(PromptInjectionDetector):
    ...
    def __init__(
        ...
        # Some HF repos (e.g. leolee99/PIGuard) ship only model weights without tokenizer files;
        # in that case the caller must pass tokenizer_name pointing to a compatible base model
        # and we load the tokenizer explicitly because pipeline() does not auto-load it from a string here.
        pipeline_kwargs: dict = {"model": self.model_name, "device": device, "trust_remote_code": True}
        if tokenizer_name is not None:
            pipeline_kwargs["tokenizer"] = AutoTokenizer.from_pretrained(tokenizer_name, use_fast=False)
        self.pipeline = pipeline("text-classification", **pipeline_kwargs)

    def detect(self, tool_output: str) -> tuple[bool, float]:
        result: dict = self.pipeline(tool_output)[0]  # type: ignore
        safety_score: float = (
            result["score"] if result["label"].lower() == self.safe_label.lower() else 1 - result["score"]
        )
