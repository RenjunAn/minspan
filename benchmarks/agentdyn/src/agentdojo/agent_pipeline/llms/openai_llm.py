...
_SYSTEM_ROLE_MODELS = {"qwen3-max"}
_REASONING_CONTENT_MODELS = set()
...
def _uses_system_role(model_name: str) -> bool:
    # Some OpenAI-compatible APIs do not support developer messages.
    return _is_deepseek_model(model_name) or model_name in _SYSTEM_ROLE_MODELS


def _replays_reasoning_content(model_name: str) -> bool:
    return _is_deepseek_model(model_name) or model_name in _REASONING_CONTENT_MODELS
...
def _message_to_openai(message: ChatMessage, model_name: str) -> ChatCompletionMessageParam:
    match message["role"]:
        case "system":
            if _uses_system_role(model_name):
        ...
        case "assistant":
            ...
            if _replays_reasoning_content(model_name) and message.get("reasoning_content") is not None:
                assistant_message["reasoning_content"] = message["reasoning_content"]  # type: ignore
...
def _openai_to_assistant_message(message: ChatCompletionMessage) -> ChatAssistantMessage:
    ...
    assistant_message = ChatAssistantMessage(
        role="assistant", content=_assistant_message_to_content(message), tool_calls=tool_calls
    )
...
class OpenAILLMToolFilter(BasePipelineElement):
    ...
    def query(
        ...
        completion = self.client.chat.completions.create(
            ...
            **({"extra_body": {"thinking": {"type": "disabled"}}} if _is_deepseek_model(self.model) else {}),
        )
        output = _openai_to_assistant_message(completion.choices[0].message)
