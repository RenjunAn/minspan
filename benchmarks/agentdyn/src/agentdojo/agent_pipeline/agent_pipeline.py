...
from agentdojo.agent_pipeline.data_filter import DATA_FILTER_DEFAULT_MODEL, DataFilterDefense
...
from agentdojo.agent_pipeline.pi_sanitizer import DEEPSEEK_FLASH_PI_SANITIZER_PROMPT, LLMPISanitizer
...
def _build_camel_pipeline(llm_name: str, defense: str) -> BasePipelineElement:
    ...
    if (
        ...
        or llm_name.startswith("o1")
        or llm_name.startswith("o3")
        or llm_name.startswith("o4")
    ):
        provider = "openai"
        ...
        raise ValueError(f"'{defense}' currently supports OpenAI/Google/Anthropic models. Got model '{llm_name}'.")
...
def _build_drift_pipeline(
    ...
    client = _make_drift_client(llm_name, drift_logger)
...
def _make_drift_client(llm_name: str, drift_logger: logging.Logger):
    _add_repo_to_syspath("DRIFT")
    from client import DeepSeekModel, GoogleModel, OpenAIModel, OpenRouterModel  # type: ignore
    ...
    if (
        ...
        return OpenAIModel(model=llm_name, logger=drift_logger)
    if llm_name.startswith("gemini"):
        return GoogleModel(model=llm_name, logger=drift_logger)
    if llm_name.startswith("deepseek"):
        return DeepSeekModel(model=llm_name, logger=drift_logger)
    return OpenRouterModel(model=llm_name, logger=drift_logger)


def _make_deepseek_flash_pi_sanitizer() -> LLMPISanitizer:
    api_key = os.getenv("DEEPSEEK_API_KEY")
    if api_key is None:
        raise ValueError("DEEPSEEK_API_KEY must be set to use DeepSeek Flash PI sanitizer")
    client = openai.OpenAI(
        api_key=api_key,
        base_url="https://api.deepseek.com",
    )
    return LLMPISanitizer(
        client=client,
        ...
        prompt_id="gepa_300_sas_container_patch_v1",
    )


def _make_data_filter_defense() -> DataFilterDefense:
    return DataFilterDefense(
        model_path=os.getenv("DATAFILTER_MODEL_PATH", DATA_FILTER_DEFAULT_MODEL),
        tensor_parallel_size=int(os.getenv("DATAFILTER_TENSOR_PARALLEL_SIZE", "1")),
        dtype=os.getenv("DATAFILTER_DTYPE", "bfloat16"),
        max_model_len=_optional_int_env("DATAFILTER_MAX_MODEL_LEN"),
        gpu_memory_utilization=_optional_float_env("DATAFILTER_GPU_MEMORY_UTILIZATION"),
        max_tokens=int(os.getenv("DATAFILTER_MAX_TOKENS", "8192")),
    )
...
def _required_env(name: str) -> str:
    ...
    if not value:
        raise ValueError(f"{name} must be set to use the token tagger defense")
    return value
...
def _tagger_runtime_config(
    batch_size_env: str,
    *,
    default_batch_size: int,
) -> tuple[str, int]:
    device = os.getenv("TAGGER_DEVICE", "cuda")
    batch_size = int(
        os.getenv(
            batch_size_env,
            os.getenv("TAGGER_BATCH_SIZE", str(default_batch_size)),
        )
    )
    if batch_size < 1:
        raise ValueError(f"{batch_size_env} must be >= 1")
    return device, batch_size


def _make_datafilter_bidir_tagger_defense() -> ToolOutputTaggerDefense:
    checkpoint_path = _required_env("DATAFILTER_TAGGER_CHECKPOINT")
    backbone_model = _required_env("DATAFILTER_BACKBONE_MODEL")
    backbone_revision = os.getenv("DATAFILTER_BACKBONE_REVISION")
    device, batch_size = _tagger_runtime_config(
        "DATAFILTER_TAGGER_BATCH_SIZE",
    ...
    backend = TokenTaggerBackend.from_datafilter_checkpoint(
        ...
        allow_backbone_mismatch=_bool_env("DATAFILTER_ALLOW_BACKBONE_MISMATCH"),
        device=device,
...
def _make_modernbert_tagger_defense() -> ToolOutputTaggerDefense:
    checkpoint_path = _required_env("MODERNBERT_TAGGER_CHECKPOINT")
    device, batch_size = _tagger_runtime_config(
        "MODERNBERT_TAGGER_BATCH_SIZE",
...
def get_llm(provider: str, model: str, model_id: str | None, tool_delimiter: str) -> BasePipelineElement:
    if provider == "openai":
        ...
        api_key = os.getenv("DEEPSEEK_API_KEY")
        if api_key is None:
            raise ValueError("DEEPSEEK_API_KEY must be set to use DeepSeek models")
        client = openai.OpenAI(
            api_key=api_key,
            base_url="https://api.deepseek.com",
        )
...
class AgentPipeline(BasePipelineElement):
    ...
    def from_config(cls, config: PipelineConfig) -> Self:
        ...
        if config.defense == "camel":
            ...
            try:
                return cast(Self, _build_camel_pipeline(llm_name, config.defense))
        ...
        if config.defense == "drift":
            ...
            try:
                return cast(Self, _build_drift_pipeline(llm_name, tool_output_formatter))
            except ImportError as e:
                raise ImportError(
        ...
        if config.defense == "tool_filter":
            ...
            return pipeline
        if (
            config.defense == "transformers_pi_detector"
            or config.defense == "piguard_detector"
            or config.defense == "prompt_guard_2_detector"
        ):
            ...
            pipeline.name = f"{llm_name}-{config.defense}"
            return pipeline
        if config.defense == "deepseek_flash_pi_sanitizer":
            tools_loop = ToolsExecutionLoop(
                [
                    ToolsExecutor(tool_output_formatter),
                    _make_deepseek_flash_pi_sanitizer(),
                    llm,
                ]
            )
            pipeline = cls(
        ...
        if config.defense == "data_filter":
            json_tool_output_formatter = partial(tool_result_to_str, dump_fn=json.dumps)
            tools_loop = ToolsExecutionLoop(
                [
                    ToolsExecutor(json_tool_output_formatter),
                    _make_data_filter_defense(),
                    llm,
                ]
            )
            pipeline = cls(
        ...
        if config.defense in (
            ...
            pipeline.name = f"{llm_name}-{config.defense}-{tagger_defense.run_id}"
            return pipeline
