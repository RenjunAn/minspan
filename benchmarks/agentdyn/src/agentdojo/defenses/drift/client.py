...
class OpenRouterModel():
    ...
    def _completion_options(self, thinking_type=None):
        options = {self.max_tokens_parameter: 8000}
        if thinking_type is not None:
            options["extra_body"] = {"thinking": {"type": thinking_type}}
        return options

    def agent_run(self, messages, tools=[], query=None, initial_trajectory=None, achieved_trajectory=None, node_checklist=None, name="default", thinking_type=None):
        """
        Employ the LLM to response the prompt.
        """
        for message in messages:
        ...
        response = self.client.chat.completions.create(
            model=self.model,
            messages=messages,
            **self._completion_options(thinking_type),
        )
        self.last_reasoning_content = getattr(response.choices[0].message, "reasoning_content", None)
    ...
    def llm_run(self, SystemPrompt, UserPrompt, name="default"):
        ...
        try:
            response = self.client.chat.completions.create(
                ...
                **self._completion_options(getattr(self, "llm_run_thinking_type", None)),
            ) 
            response_content = response.choices[0].message.content
            self.last_reasoning_content = getattr(response.choices[0].message, "reasoning_content", None)
...
class DeepSeekModel(OpenRouterModel):
    ...
    def __init__(self, model="deepseek-v4-flash", api_key=None, api_base="", api_version="2024-10-21", logger=None):
        self.api_base = api_base
        self.api_key = api_key
        self.model = model
        self.logger = logger
        ...
        if not api_key:
            raise ValueError("DEEPSEEK_API_KEY must be set to use DeepSeek with DRIFT.")
        ...
        self.last_reasoning_content = None
        self.llm_run_thinking_type = "disabled"
