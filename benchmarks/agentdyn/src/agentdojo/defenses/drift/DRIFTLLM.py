...
class DRIFTLLM(PromptingLLM):
    ...
    def _with_client_reasoning_content(self, output):
        reasoning_content = getattr(self.client, "last_reasoning_content", None)
        if reasoning_content is not None:
            output["reasoning_content"] = reasoning_content
        return output

    def _defense_agent_run(self, messages, tools, **kwargs):
        if getattr(self.client, "supports_thinking_control", False):
            kwargs["thinking_type"] = "disabled"
        return self.client.agent_run(messages, tools, **kwargs)
    ...
    def query(
        ...
        if self.args.build_constraints:
            if len(openai_messages) < 2:
                ...
                completion = self._defense_agent_run(openai_messages, self.tools_docs_list)
        ...
        output = self._with_client_reasoning_content({"role": "assistant", "content": completion[0] or "", "tool_calls": []})
        ...
        for _ in range(self._MAX_ATTEMPTS):
            try:
                output = self._with_client_reasoning_content(self._parse_model_output(completion[0]))
                break
