...
def api_request(sys_prompt, user_prompt, temperature=0.0) -> None:
    ...
    elif policy_model.startswith("deepseek"):
        deepseek_api_key = os.getenv("DEEPSEEK_API_KEY")
        if not deepseek_api_key:
            raise ValueError("DEEPSEEK_API_KEY must be set to use DeepSeek as policy model")
        client = OpenAI(base_url="https://api.deepseek.com", api_key=deepseek_api_key)
    else:
        client = OpenAI()
    deepseek_extra = {"extra_body": {"thinking": {"type": "disabled"}}} if policy_model.startswith("deepseek") else {}
...
def get_SYS_PROMPT() -> str:
    ...
    output_formater = ""
    if policy_model.startswith("o1") or policy_model.startswith("o3") or policy_model.startswith("gpt-4.1") or policy_model.startswith("gemini") or policy_model.startswith("meta-llama/") or policy_model.startswith("Qwen/") or policy_model.startswith("qwen.") or policy_model.startswith("moonshotai") or policy_model.startswith("deepseek"):
...
def get_SYS_PROMPT_2() -> str:
    ...
    if policy_model.startswith("gemini") or policy_model.startswith("meta-llama/") or policy_model.startswith("Qwen/") or policy_model.startswith("qwen.") or policy_model.startswith("moonshotai") or policy_model.startswith("deepseek"):
        sys_prompt = sys_prompt[:-1]
