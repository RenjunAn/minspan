...
from ...evaluations import DEFAULT_JUDGE_MODEL, llm_judge, llm_judge_batch
...
def get_cached_strategy_search_judge_llm(model_name_or_path: str = DEFAULT_JUDGE_MODEL):
    """Return an already loaded strategy-search vLLM backend suitable for llm_judge."""
    for model in (JUDGE_VLLM_MODEL, ATTACKER_VLLM_MODEL):
        if (
            model is not None
            and getattr(model, "model_name_or_path", None) == model_name_or_path
        ):
            return model
    return None
...
class StrategySearchAttacker:
    ...
    def _get_judge_fallback_llm(self):
        if getattr(self.target_llm, "model_name_or_path", None) == DEFAULT_JUDGE_MODEL:
            return self.target_llm
        if self._get_attacker_model_path() == DEFAULT_JUDGE_MODEL:
            return self._ensure_attacker_llm()
        return None

    def _get_llm_judge_kwargs(self, evaluator):
        if getattr(evaluator, "__name__", "") != "llm_judge":
            return {}
        if self.judge_vllm is not None:
            return {"llm": self.judge_vllm}
        fallback_llm = self._get_judge_fallback_llm()
    ...
    def _confirm_candidate_with_main_path(
        ...
        fitness = float(
            evaluator(
                ...
                **self._get_llm_judge_kwargs(evaluator),
            )
        )
