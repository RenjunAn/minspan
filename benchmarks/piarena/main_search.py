...
def get_preferred_search_judge_llm(attack_name):
    if attack_name != "strategy_search":
        return None
    from piarena.attacks.strategy_search.attack_strategy_search import (
        get_cached_strategy_search_judge_llm,
    )

    return get_cached_strategy_search_judge_llm()

def parse_args():
    parser = argparse.ArgumentParser(prog='PIArena', description="test")
    attack_choices = ["strategy_search", "pair", "tap", "nanogcg"]
    ...
    parser.add_argument('--attacker_llm', type=str, default=None,
...
def main(args):
    ...
    for idx, dp in tqdm(enumerate(dataset)):
        ...
        preferred_judge_llm = get_preferred_search_judge_llm(args.attack)
        result_dp["utility"] = utility_evaluator(
            ...
            **get_reusable_judge_llm_kwargs(
                ...
                args.backend_llm,
                llm,
                preferred_llm=preferred_judge_llm,
            ),
        )
