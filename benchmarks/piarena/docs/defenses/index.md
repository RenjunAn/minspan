...
- [`deepseek_pisanitizer`](/docs/defenses/deepseek-pisanitizer): LLM-based context sanitization with a DeepSeek-compatible endpoint
- [`modernbert_tagger`](/docs/defenses/modernbert-tagger): ModernBERT token-level sanitization
...
- Try `pisanitizer`, `datafilter`, `promptarmor`, `promptlocate`, `deepseek_pisanitizer`, or `modernbert_tagger` when you want cleaned context instead of a warning-only block.
