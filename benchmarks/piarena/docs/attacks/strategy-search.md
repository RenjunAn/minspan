...
When the attacker model is the default LLM judge model (`Qwen/Qwen3-4B-Instruct-2507`), `strategy_search` reuses its cached vLLM backend for candidate judging and final `main_search.py` utility/ASR judging. This avoids loading an additional Transformers judge model during single-GPU search runs.
