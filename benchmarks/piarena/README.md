...
- `deepseek_pisanitizer` - AgentDyn DeepSeek LLM sanitizer for untrusted context and tool outputs
- `modernbert_tagger` - AgentDyn ModernBERT token tagger for span-level sanitization

<!-- You can go to [this link]() to see all supported datasets, attacks and defenses. -->

### 🔍 Search-based Attacks

PIArena supports search-based attacks (PAIR, TAP, Strategy Search, and GCG/nanoGCG) that iteratively refine injected prompts using an attack LLM or the backend model. Use `main_search.py` for these attacks:

```bash
# --attack can be tap, pair, strategy_search, nanogcg
...
# Merged AgentDyn suite with a config-driven PIArena defense
...
`defense_config` from YAML files is supported by the standard, search, InjecAgent, AgentDojo, and AgentDyn runners.
...
#### DeepSeek and ModernBERT Smoke and Leaderboard Runs

The DeepSeek PISanitizer and ModernBERT tagger integrations include defense-specific runners that mirror the non-WASP leaderboard scope:

```bash
# One-sample smoke coverage across standard, external, InjecAgent, AgentDojo, and AgentDyn scopes
python scripts/run_deepseek_pisanitizer_smoke.py --dry-run
python scripts/run_modernbert_tagger_smoke.py --skip-agents

# Full non-WASP leaderboard matrix
python scripts/run_deepseek_pisanitizer_full.py --dry-run
python scripts/run_modernbert_tagger_full.py
```

Each script accepts `--config` to override its default YAML. The full runners write leaderboard-compatible entries to `results/leaderboard_entries/{name}.json` using the same field names as `website/data/results.json`. InjecAgent utility is derived from `valid_rate`; AgentDojo and AgentDyn ASR is derived as `100 - average security`.
