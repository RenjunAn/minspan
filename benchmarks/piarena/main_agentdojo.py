...
from piarena.config import load_experiment_config, merge_config_args
...
def redact_sensitive_config(config: dict | None) -> dict:
    """Return a copy of config safe for logging."""
...
def run_agentdojo_benchmark(args, model_type: str, azure_model_name: str = None):
    ...
    if args.defense != "none":
        ...
        env["PIARENA_DEFENSE_CONFIG"] = json.dumps(args.defense_config or {})
    ...
    if args.defense != "none":
        ...
        print(f"[Run] PIARENA_DEFENSE_CONFIG={json.dumps(redact_sensitive_config(args.defense_config))}")
    print()
...
def main(args):
    ...
    print(f"  Force:    {args.force_rerun}")
    print("="*60)
...
if __name__ == "__main__":
    ...
    attack_choices = ["none", "direct", "important_instructions", "tool_knowledge", "injecagent"]
    defense_choices = ["none", "datafilter", "pisanitizer", "promptguard",
                       "datasentinel", "piguard", "attentiontracker",
                       "promptarmor", "promptlocate",
                       "deepseek_pisanitizer", "modernbert_tagger"]
    suite_choices = ["workspace", "slack", "travel", "banking",
    ...
    parser.add_argument("--force-rerun", "-f", action="store_true", default=None,
                        help="Re-run AgentDojo tasks even if matching log files already exist.")
    ...
    merge_config_args(args, file_config, defaults)
    if isinstance(args.suite, str):
        args.suite = [args.suite]
    if isinstance(args.user_tasks, str):
        args.user_tasks = [args.user_tasks]
    if args.attack not in attack_choices:
        parser.error(f"argument --attack: invalid choice: {args.attack!r} (choose from {attack_choices})")
    if args.defense not in defense_choices:
        parser.error(f"argument --defense: invalid choice: {args.defense!r} (choose from {defense_choices})")
    if args.suite is not None:
        invalid_suites = [suite for suite in args.suite if suite not in suite_choices]
        if invalid_suites:
            parser.error(f"argument --suite: invalid choices: {invalid_suites!r} (choose from {suite_choices})")
