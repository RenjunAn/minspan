...
# backend, paper setting). Requires checkpoints/minspan and the AgentDyn
# environment (cd benchmarks/agentdyn && uv sync).
set -euo pipefail
ROOT="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")/.." && pwd)"
export MODERNBERT_TAGGER_CHECKPOINT="${MODERNBERT_TAGGER_CHECKPOINT:-$ROOT/checkpoints/minspan}"
